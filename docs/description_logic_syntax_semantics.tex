...
Queries are intermediate finite relations with ordered, named columns and set
semantics: duplicate tuples are removed. They are available in the base,
extended, and unsolvability families, while the final feature categories remain
concept, role, Boolean, and numerical. A query occurs inside a projection,
\texttt{b\_nonempty}, or \texttt{n\_count}; it is not a standalone feature or
a grammar-enumeration category.
...
\midrule
\endhead
\texttt{(q\_atomic\_state "p" (x ...))} &
Current tuples of predicate $p$, with one distinct column label per argument. \\
\texttt{(q\_atomic\_goal "p" true (x ...))} &
Positive goal tuples of $p$; \texttt{false} selects explicitly negative goal tuples. \\
\texttt{(q\_concept x C)} &
Lift $C$ into a one-column relation named $x$. \\
\texttt{(q\_role (x y) R)} &
Lift $R$ into a two-column relation named $x,y$. \\
\texttt{(q\_join Q1 Q2)} &
Natural join on all shared column labels; a Cartesian product if none are shared. \\
\texttt{(q\_project (x ...) Q)} &
Keep the named columns in the specified order and eliminate duplicate tuples. \\
\texttt{(q\_rename (x ...) Q)} &
Replace all column labels positionally, preserving arity and tuples. \\
\texttt{(q\_select\_equal x y Q)} &
Keep tuples whose $x$ and $y$ values are equal, preserving all columns. \\
\texttt{(q\_select\_value x "a" Q)} &
Keep tuples whose $x$ value is the domain constant $a$, preserving all columns. \\
\texttt{(q\_union Q1 Q2)} &
Set union; both operands must have identical ordered schemas. \\
\texttt{(q\_difference Q1 Q2)} &
Set difference; both operands must have identical ordered schemas. \\
\texttt{(c\_project x Q)} &
The concept containing the distinct values of column $x$. \\
\texttt{(r\_project x y Q)} &
The role containing the distinct ordered pairs of values from columns $x,y$. \\
\texttt{(b\_nonempty Q)} &
True exactly when the query has at least one tuple. \\
\texttt{(n\_count Q)} &
The number of distinct query tuples. \\
\bottomrule
\end{longtable}
\end{compacttable}

Column labels are unquoted identifiers. Referenced columns must exist;
atomic, lifted, projected, and renamed schemas use distinct labels. Repeated
predicate arguments are expressed using distinct labels followed by
\texttt{q\_select\_equal}. Use \texttt{q\_rename} or \texttt{q\_project} to align
schemas before a union or difference. Selection by value resolves domain
constants, not problem-specific objects.

Nullary predicates use the empty schema \texttt{()}. A relation on this schema
is either empty or contains the single empty tuple. Consequently,
\texttt{(q\_project () Q)} represents existence: its count is zero or one.
Projection removes duplicate tuples even when discarded columns distinguished
the input rows.

For example, this concept joins a ternary predicate with a unary concept and
keeps the first argument:
\begin{verbatim}
(c_project item
  (q_join
    (q_atomic_state "stored" (item shelf room))
    (q_concept room (c_atomic_state "accessible"))))
\end{verbatim}
Each query operation contributes one to syntactic complexity, as does its
containing bridge; concept and role children of lifts count recursively.
Column labels, predicate names, and constant names do not add to complexity.
...
(load, choose, do, sketch, or call).  Edges that only expose control failure or an open
...
\]
Load and choose rules update registers and internal memory, do and sketch rules
may change $\bar{s}$ through a planning transition, and call rules push the caller
frame before entering the callee module.  Execution configurations also retain
module arguments and the complete caller stack.

Both binding rules admit an optional trailing \texttt{(:effects ...)} body,
using the same Boolean and numerical effects as do and sketch rules.  For
example,
\begin{verbatim}
(:choose (:conditions) (:concept candidates)
  (:register (:concept selected)) (:effects (increases selected_count)))
\end{verbatim}
Conditions and the candidate denotation $D$ are evaluated before binding.  The
admitted candidates are
\[
 D_E = \{o\in D\mid E((\bar{s},\rho),(\bar{s},\rho[r\leftarrow o]))\}.
\]
The planning state stays unchanged: effects compare features under the old and
new register valuations.  Concept rules bind one object and role rules bind an
object pair.  Omitted or empty effects admit every candidate; unmentioned
features impose no additional constraints.

With \texttt{universal=false}, ordinary successor selection stays greedy.  A
selected choose rule instead tries its admitted bindings until a complete
continuation reaches a goal.  With \texttt{universal=true}, every ordinary
successor must succeed, while each enabled choose rule needs at least one
successful binding.  Distinct choose rules remain separate obligations.  A
choose rule whose conditions hold but whose admitted denotation is empty fails;
it does not silently return from its module.

One worklist executor handles every rule kind.  Ordinary rules keep the existing
greedy selection or exhaustive universal traversal.  A choose point saves the
pending work, visitation and transition trails, and current plan length.  On
downstream failure, execution restores that snapshot, truncates the plan, and
retries the next binding.  Choice points survive module returns.  A cycle among
the transitions selected in an attempt rejects that attempt and permits another
binding.  Time and state limits apply across all retries;
exhausting a resource reports the corresponding limit rather than a failed
binding.

The proof graph records explored attempts, including rejected branches.  For
programs containing choose, its diagnostic deadends or cycles may therefore
remain even when execution succeeds; success requires an attempt in which every
required continuation succeeds and the selected transitions are acyclic.  The
\texttt{deadend\_states} list may include
the source of a failed choose obligation.  An empty choice has no binding edge
and needs no synthetic failure state.  Vertex labels \texttt{is\_alive} and
\texttt{is\_unsolvable} record reached-state and classifier information; neither
those labels nor the diagnostic lists determine the overall result.  Programs
without choose retain the existing
execution semantics.  Structural termination analysis treats choose like load:
explicit effects constrain register-dependent features, and register-independent
features still unconstrained by those effects are preserved implicitly.  This
analyzes forward execution, not solver completeness.  Successful execution on
training tasks is empirical evidence; it does not prove that the learned
binding denotations and segments preserve a solution on every instance.
