...
void tag_invoke(boost::json::value_from_tag,
                ...
                const View<Index<runir::kr::dl::Concept<Family, Tag>>, C>& value,
                Dictionaries* dictionaries)
{
    dictionaries->object(result, value, [&](auto& ar)
    {
        ...
        if constexpr (requires { value.get_object(); })
            ar.field("object", value.get_object());
        ...
        if constexpr (requires { value.get_objects(); })
            ar.field("objects", value.get_objects());
        if constexpr (requires { value.get_n(); })
            ar.field("n", value.get_n());
        if constexpr (requires { value.get_role(); })
            ar.field("role", value.get_role());
        if constexpr (requires { value.get_concept(); })
            ar.field("concept", value.get_concept());
