...
public:
    ...
    explicit SparseGraph(const G& graph) : SparseGraph()
    {
        ...
        auto vertex_to_dense = ygg::UnorderedMap<VertexIndex, ygg::uint_t>();
        ...
        auto edge_set = std::set<std::pair<ygg::uint_t, ygg::uint_t>>();
        ...
        for (ygg::uint_t vertex = 0; vertex < vertices.size(); ++vertex)
        {
    ...
    auto get_coloring() const -> const std::vector<ygg::uint_t>&;
