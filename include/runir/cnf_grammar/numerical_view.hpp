...
#include "runir/cnf_grammar/numerical_data.hpp"
...
class View<Index<runir::cnf_grammar::Numerical<Tag>>, C>
{
private:
    const C* m_context;
    Index<runir::cnf_grammar::Numerical<Tag>> m_handle;

public:
    View(Index<runir::cnf_grammar::Numerical<Tag>> handle, const C& context) noexcept : m_context(&context), m_handle(handle) {}
    ...
    auto get_arg() const noexcept
        requires std::same_as<Tag, runir::CountTag>
    {
        return make_view(get_data().arg, *m_context);
    }

    auto get_lhs() const noexcept
        requires std::same_as<Tag, runir::DistanceTag>
    {
