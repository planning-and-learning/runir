...
{
    using Entity = kr::dl::cnf_grammar::DerivationRule<Family, Category>;
    using View = ygg::View<ygg::Index<Entity>, kr::dl::cnf_grammar::ConstructorRepositoryFor<Family>>;
    static_assert(std::same_as<View, kr::dl::cnf_grammar::FamilyDerivationRuleView<Family, Category>>);
    if constexpr (std::same_as<Family, kr::BaseFamilyTag>)
        return std::same_as<View, kr::dl::cnf_grammar::BaseDerivationRuleView<Category>>;
    else
        return true;
}
...
static_assert(indexed_data_views<kr::BaseFamilyTag>(kr::dl::cnf_grammar::FamilyDerivationRuleTypes<kr::BaseFamilyTag> {}));
static_assert(indexed_data_views<kr::ExtFamilyTag>(kr::dl::cnf_grammar::FamilyDerivationRuleTypes<kr::ExtFamilyTag> {}));
static_assert(indexed_data_views<kr::UnsFamilyTag>(kr::dl::cnf_grammar::FamilyDerivationRuleTypes<kr::UnsFamilyTag> {}));
