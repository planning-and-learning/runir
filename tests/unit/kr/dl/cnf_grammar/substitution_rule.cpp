...
#include <runir/kr/dl/cnf_grammar/substitution_rule_index.hpp>
#include <runir/kr/dl/cnf_grammar/substitution_rule_view.hpp>
...
{
    using Entity = kr::dl::cnf_grammar::SubstitutionRule<Family, Category>;
    using View = ygg::View<ygg::Index<Entity>, kr::dl::cnf_grammar::ConstructorRepositoryFor<Family>>;
    static_assert(std::same_as<View, kr::dl::cnf_grammar::FamilySubstitutionRuleView<Family, Category>>);
    if constexpr (std::same_as<Family, kr::BaseFamilyTag>)
        return std::same_as<View, kr::dl::cnf_grammar::BaseSubstitutionRuleView<Category>>;
    else
        return true;
}
...
static_assert(indexed_data_views<kr::BaseFamilyTag>(kr::dl::cnf_grammar::FamilySubstitutionRuleTypes<kr::BaseFamilyTag> {}));
static_assert(indexed_data_views<kr::ExtFamilyTag>(kr::dl::cnf_grammar::FamilySubstitutionRuleTypes<kr::ExtFamilyTag> {}));
static_assert(indexed_data_views<kr::UnsFamilyTag>(kr::dl::cnf_grammar::FamilySubstitutionRuleTypes<kr::UnsFamilyTag> {}));
