...
{
    using Entity = kr::dl::cnf_grammar::Constructor<Family, Category>;
    using View = ygg::View<ygg::Index<Entity>, kr::dl::cnf_grammar::ConstructorRepositoryFor<Family>>;
    static_assert(std::same_as<View, kr::dl::cnf_grammar::FamilyConstructorView<Family, Category>>);
    if constexpr (std::same_as<Family, kr::BaseFamilyTag>)
        return std::same_as<View, kr::dl::cnf_grammar::BaseConstructorView<Category>>;
    else
        return true;
}
...
static_assert(indexed_data_views<kr::BaseFamilyTag>(kr::dl::cnf_grammar::FamilyConstructorTypes<kr::BaseFamilyTag> {}));
static_assert(indexed_data_views<kr::ExtFamilyTag>(kr::dl::cnf_grammar::FamilyConstructorTypes<kr::ExtFamilyTag> {}));
static_assert(indexed_data_views<kr::UnsFamilyTag>(kr::dl::cnf_grammar::FamilyConstructorTypes<kr::UnsFamilyTag> {}));
