                         ...
                         const std::filesystem::path& task_file) -> datasets::TaskSearchContextPtr<tyr::planning::GroundTag>
{
    ...
    auto lifted_task = p::Task<p::LiftedTag>::create(fp::Parser(domain_file).parse_task(task_file));
    auto task = lifted_task->instantiate_ground_task(*execution_context).task;
    return datasets::TaskSearchContext<p::GroundTag>::create(std::move(task), std::move(execution_context));
}
                         ...
                         const std::filesystem::path& task_file) -> datasets::TaskSearchContextPtr<tyr::planning::LiftedTag>
{
    ...
    auto execution_context = ygg::ExecutionContext::create(1);
    auto task = p::Task<p::LiftedTag>::create(fp::Parser(domain_file).parse_task(task_file));
    return datasets::TaskSearchContext<p::LiftedTag>::create(std::move(task), std::move(execution_context));
}
