...
make_result(const ModuleAnalysis& analysis, const std::vector<runir::kr::ps::detail::PolicyEdge>& edges, const runir::kr::ps::detail::SieveResult& sieve_result)
{
    ...
    const auto map_vertex = [&](std::size_t vertex)
    {
        ...
        {
            auto numericals = runir::kr::ps::detail::vertex_numericals(vertex, analysis.policy);
            vertex_remap[vertex] =
                counterexample_builder.add_vertex(ModulePolicyGraphVertexLabel(runir::kr::ps::detail::vertex_booleans(vertex, analysis.policy),
                                                                               std::move(numericals),
                                                                               analysis.memory_states[vertex % analysis.policy.num_memory_states]));
        }
    ...
    for (const auto& edge : edges)
    {
        ...
        counterexample_builder.add_directed_edge(
            ...
            ModulePolicyGraphEdgeLabel(analysis.rules[edge.rule_position], analysis.policy.rule_profiles[edge.rule_position].numerical_changes));
    }
