...
summarize_opposing_changes(const std::vector<PolicyEdge>& edges, const SketchAnalysis& analysis, const StrongComponents& components)
{
    ...
    for (const auto& edge : edges)
    {
        ...
        const auto& changes = analysis.profiles[edge.rule_position].numerical_changes;
        ...
        const auto source_booleans = vertex_booleans(edge.source, num_booleans);
        const auto target_booleans = vertex_booleans(edge.target, num_booleans);
...
{
    const auto num_vertices = std::size_t { 1 } << (analysis.features.booleans.size() + analysis.features.numericals.size());
    ...
    while (removed_edges)
    {
        ...
        const auto component_profiles = summarize_opposing_changes(edges, analysis, components);
        removed_edges = remove_unopposed_edges(edges, analysis, components, component_profiles);
    }
